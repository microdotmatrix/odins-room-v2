...
<div class="container max-w-none px-8 lg:px-[4vw] lg:py-4 2xl:px-[12vw]">
	<div class="grid gap-6 sm:grid-cols-12">
		<!-- card one -->
		{#each $albums.slice(1, 2) as album}
			<div class="col-span-12 sm:col-span-7 sm:self-end md:col-span-8 lg:col-span-5 lg:col-start-3">
				<AlbumCard {album} />
			</div>
		{/each}
		<!-- card two -->
		...
		{#each $albums.slice(7, 8) as album}
			<div class="col-span-12 sm:col-span-5 sm:self-end md:col-span-4 lg:col-span-3 lg:col-start-3">
				<AlbumCard {album} />
			</div>
		{/each}
